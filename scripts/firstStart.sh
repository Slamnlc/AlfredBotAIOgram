...
sudo docker-compose exec -T db psql -U postgres -f -C /var/lib/postgresql/alfred.sql
sudo docker-compose exec -T db psql -U postgres -f -C /var/lib/postgresql/yap.sql
